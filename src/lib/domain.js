...
export function dateRange(end, count = 7) {
...
export const mean = (values) =>
  values.length ? Math.round(values.reduce((sum, n) => sum + n, 0) / values.length) : null;

const number = (key, label, min, max, unit, step = 1) => ({
  key,
  label,
  min,
  max,
...
export const PILLARS = [
  {
    ...
    icon: 'leaf',
    color: 'green',
    description: 'Pequeñas decisiones que te nutren.',
    fields: [
      number('portions', 'Frutas y verduras', 0, 10, 'porciones'),
      number('quality', 'Calidad de tu alimentación', 1, 10, '/ 10'),
      number('water', 'Agua', 0, 12, 'vasos'),
    ],
  },
  {
    key: 'actividad',
    ...
    icon: 'activity',
    color: 'blue',
    description: 'Muévete a tu propio ritmo.',
    fields: [
      number('steps', 'Pasos', 0, 100000, 'pasos'),
      number('minutes', 'Ejercicio', 0, 600, 'minutos'),
      number('intensity', 'Intensidad percibida', 1, 10, '/ 10'),
    ],
  },
  {
    key: 'sueno',
    ...
    icon: 'moon',
    color: 'purple',
    description: 'Dale espacio a tu descanso.',
    fields: [
      number('hours', 'Tiempo de sueño', 0, 24, 'horas', 0.25),
      number('quality', 'Calidad del descanso', 1, 10, '/ 10'),
      { key: 'bedtime', label: 'Hora de acostarte (opcional)', type: 'time' },
    ],
  },
  {
    key: 'sustancias',
    ...
    icon: 'shield',
    color: 'orange',
    description: 'Observa tus hábitos sin juicios.',
    fields: [
      number('cigarettes', 'Cigarrillos', 0, 100, 'unidades'),
      number('alcohol', 'Alcohol', 0, 30, 'unidades', 0.5),
    ],
  },
  {
    key: 'estres',
    ...
    icon: 'sun',
    color: 'yellow',
    description: 'Haz una pausa para escucharte.',
    fields: [
      number('stress', 'Estrés percibido', 1, 10, '/ 10'),
      number('minutes', 'Respiración o meditación', 0, 180, 'minutos'),
    ],
  },
  {
    key: 'conexion',
    ...
    icon: 'heart',
    color: 'pink',
    description: 'Cuida lo que te hace bien.',
    fields: [
      number('connection', 'Conexión con otras personas', 1, 10, '/ 10'),
      number('minutes', 'Actividad con propósito', 0, 600, 'minutos'),
    ],
  },
];
...
export function validatePillar(pillar, values) {
  for (const f of pillar.fields.filter((f) => f.type === 'number')) {
    ...
    if (
      ...
      return `${f.label}: ingresa un valor entre ${f.min} y ${f.max}.`;
  }
  if (values.bedtime && !/^([01][0-9]|2[0-3]):[0-5][0-9]$/.test(values.bedtime))
...
export function scorePillar(key, values) {
  ...
  const scores = {
    ...
    sustancias: () =>
      mean([clamp(100 - (v.cigarettes / 20) * 100), clamp(100 - (v.alcohol / 10) * 100)]),
...
export function habitSummary(records) {
  ...
  const daily = [...byDay]
    ...
    .map(([date, scores]) => ({ date, score: mean([...scores.values()]), coverage: scores.size }));
  const pillars = PILLARS.map((p) => ({
    ...p,
    score: mean([...byDay.values()].filter((day) => day.has(p.key)).map((day) => day.get(p.key))),
  }));
  const first = byDay.get(daily[0]?.date);
  const last = byDay.get(daily.at(-1)?.date);
  const common = first && last ? [...first.keys()].filter((key) => last.has(key)) : [];
  const trend =
    daily.length < 2 || !common.length
      ? null
      : mean(common.map((key) => last.get(key))) - mean(common.map((key) => first.get(key)));
  return {
    score: mean(daily.map((d) => d.score)),
    daily,
    pillars,
    days: daily.length,
    trend,
    trendCoverage: common.length,
    coverage: daily.reduce((total, day) => total + day.coverage, 0),
  };
}
export function streak(records, today) {
  const dates = new Set(
    records.filter((r) => PILLARS.some((p) => p.key === r.pillar)).map((r) => r.date),
  );
...
export function adherence(tasks, completions, dates) {
  const days = dates.map((date) => {
    ...
    const completed = scheduled.filter((t) =>
      completions.some((c) => c.task_id === t.id && c.completion_date === date && c.completed),
    ).length;
    return {
      date,
      scheduled: scheduled.length,
      completed,
      score: scheduled.length ? Math.round((completed / scheduled.length) * 100) : null,
    };
  });
...
export function validateTask(task) {
  ...
  if (!TASK_TYPES[task.task_type] || !FREQUENCIES[task.frequency])
    return 'Selecciona un tipo y una frecuencia válidos.';
  if (!isValidDay(task.start_date)) return 'Selecciona la fecha de inicio.';
  if (task.end_date && (!isValidDay(task.end_date) || task.end_date < task.start_date))
    return 'El término debe ser igual o posterior al inicio.';
  if (
    task.frequency === 'weekly' &&
    (!task.days_of_week?.length ||
      task.days_of_week.some((d) => !Number.isInteger(d) || d < 1 || d > 7))
  )
    return 'Selecciona al menos un día.';
  if ((task.description || '').length > 1000 || (task.instructions || '').length > 1000)
    return 'Las indicaciones admiten hasta 1.000 caracteres.';
  if ((task.measurement_unit || '').length > 30) return 'La unidad admite hasta 30 caracteres.';
  if (task.task_type === 'measurement' && !task.measurement_unit?.trim())
    return 'Indica la unidad de la medición.';
  return '';
}
...
export function validateProfile(form) {
  if (!form.full_name?.trim() || form.full_name.length > 120)
    return 'Ingresa un nombre de hasta 120 caracteres.';
  if (form.age !== '' && form.age != null && !Number.isInteger(Number(form.age)))
    return 'La edad debe ser un número entero.';
  for (const [key, name, min, max] of [
    ['age', 'Edad', 1, 120],
    ['weight', 'Peso', 1, 500],
    ['height', 'Estatura', 30, 260],
    ['body_fat', 'Grasa corporal', 1, 80],
  ]) {
    if (
      form[key] !== '' &&
      form[key] != null &&
      (!Number.isFinite(Number(form[key])) || Number(form[key]) < min || Number(form[key]) > max)
    )
      return `${name}: ingresa un valor entre ${min} y ${max}.`;
  }
  return '';
}
